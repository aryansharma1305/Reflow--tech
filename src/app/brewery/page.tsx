...
const ImageSlideshow = ({ images, alt }: { images: string[]; alt: string }) => {
  ...
  return (
    <div className="relative h-[550px] rounded-3xl overflow-hidden shadow-2xl group">
      ...
      />
      <motion.div 
        className="absolute bottom-6 right-6 w-16 h-16 border-b-4 border-r-4 border-blue-400 rounded-br-2xl z-10"
        initial={{ scale: 0, opacity: 0 }}
        whileInView={{ scale: 1, opacity: 1 }}
...
export default function BreweryPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50 to-blue-100 relative overflow-hidden">
                  ...
                  >
                    {[
                      ...
                      return (
                        <motion.div
                          key={idx}
                          className="bg-gradient-to-br from-blue-600 to-blue-700 p-4 rounded-xl text-white text-center shadow-lg"
      ...
      >
        <motion.div 
          className="absolute top-40 right-40 w-96 h-96 bg-blue-300/20 rounded-full blur-3xl"
          animate={{ y: [-30, 30, -30], x: [-20, 20, -20] }}
          transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
        />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
